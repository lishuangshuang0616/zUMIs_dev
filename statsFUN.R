...
splitRG_stats <- function(bccount, mem) {
  ...
  cs <- cumsum(bccount$n)
  chunkID <- findInterval(cs, seq(0, max(cs), by = maxR))
  bccount$chunkID <- chunkID

  if (any(bccount$n > maxR)) {
    warnings <- bccount[bccount$n > maxR, .(message = paste("Warning: Barcode", XC, "has more reads than allowed for the memory limit! Proceeding anyway..."))]
...
prep_samtools_stats <- function(featfile,bccount,inex,cores,samtoolsexc){
  ...
  system(paste("rm", paste(shQuote(outfiles), collapse = " ")))
  ...
  system(paste("rm", paste(shQuote(all_rgfiles), collapse = " ")))
