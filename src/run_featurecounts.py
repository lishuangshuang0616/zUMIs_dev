...
def main():
    ...
    if os.path.exists(internal_bam):
        res_int_ex = run_featurecounts_cmd(featurecounts_exec, internal_bam, exon_saf, internal_bam + ".ex", num_threads, 0, "Exon_Internal")
        current_int_bam = res_int_ex
        
        if count_introns:
             res_int_in = run_featurecounts_cmd(featurecounts_exec, internal_bam, intron_saf, internal_bam + ".in", num_threads, 0, "Intron_Internal")
    ...
    if os.path.exists(umi_bam):
        res_umi_ex = run_featurecounts_cmd(featurecounts_exec, umi_bam, exon_saf, umi_bam + ".ex", num_threads, 1, "Exon_UMI")
        current_umi_bam = res_umi_ex
        
        if count_introns:
            res_umi_in = run_featurecounts_cmd(featurecounts_exec, umi_bam, intron_saf, umi_bam + ".in", num_threads, 1, "Intron_UMI")
